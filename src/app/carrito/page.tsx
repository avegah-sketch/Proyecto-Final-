  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={vaciar} className="text-sm text-red-600 hover:text-red-700 font-medium">
          Vaciar carrito
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <Card key={item.producto.id} className="p-4 flex gap-4">
              ...
              <div className="flex-1 min-w-0">
                <Link href={`/producto/${item.producto.id}`}>
                  <h3 className="font-semibold text-gray-900 hover:text-indigo-600 transition-colors">{item.producto.nombre}</h3>
                </Link>
                <p className="text-sm text-gray-500 mt-0.5">Talla: {item.producto.talla}</p>
                <p className="text-sm text-gray-500">Precio unitario: {formatPrecio(item.producto.precio)}</p>
        ...
        <div>
          <Card className="p-5 sticky top-24">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {envio === 0 ? <span className="text-emerald-600">Gratis</span> : formatPrecio(envio)}
                </span>
              </div>
              {subtotal < 999 && (
                <p className="text-xs text-gray-400">
                  Agrega {formatPrecio(999 - subtotal)} más para envío gratis
                </p>
              )}
              <div className="border-t pt-2 mt-2">
                <div className="flex justify-between text-base">
                  <span className="font-semibold">Total</span>
                  <span className="font-bold text-indigo-600">{formatPrecio(total)}</span>
                </div>
              </div>
            </div>
            <Link href="/checkout">
            ...
            <Link href="/catalogo" className="block text-center text-sm text-gray-500 hover:text-indigo-600 mt-3">
