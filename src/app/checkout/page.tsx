...
import { formatPrecio, luhnCheck, enmascararTarjeta, generarTransactionId } from "@/lib/utils";
...
export default function CheckoutPage() {
  ...
  const [tarjeta, setTarjeta] = useState<TarjetaPago>({
    cardNumber: "",
    cardHolder: "",
    expiryMonth: "",
    expiryYear: "",
  ...
  if (items.length === 0 && paso !== "confirmacion") {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        <h1 className="text-2xl font-bold text-gray-900">No hay productos en tu carrito</h1>
  ...
  const validarTarjeta = () => {
    ...
    if (!/^\d{2}$/.test(tarjeta.expiryMonth)) errs.expiryMonth = "Mes inválido (MM)";
    if (!/^\d{2,4}$/.test(tarjeta.expiryYear)) errs.expiryYear = "Año inválido (AA)";
    if (!/^\d{3,4}$/.test(tarjeta.cvv)) errs.cvv = "CVV inválido";

    const mes = parseInt(tarjeta.expiryMonth);
    if (mes < 1 || mes > 12) errs.expiryMonth = "Mes debe ser 01-12";
    setErrores(errs);
    return Object.keys(errs).length === 0;
  };
  ...
  const handleInfoSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (validarInformacion()) setPaso("pago");
  };

  const handlePagoSimulado = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    // Simulación de pasarela de pago con fetch asíncrono
    try {
      const response = await fetch("/api/pagos/procesar", {
        ...
        body: JSON.stringify({
          monto: total,
          tarjeta: {
            numero: tarjeta.cardNumber.replace(/\s/g, ""),
            titular: tarjeta.cardHolder,
            mesExpiracion: tarjeta.expiryMonth,
            anioExpiracion: tarjeta.expiryYear,
            cvv: tarjeta.cvv,
          },
        }),
      });

      const data: ResultadoPago = await response.json();
      setResultado(data);
      ...
      setResultado({
        exito: false,
        mensaje: "Error de conexión con la pasarela de pagos. Intenta de nuevo.",
      });
    } finally {
      setProcesando(false);
    }
            ...
            <div
              className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
                paso === p
                  ...
                  : ["informacion", "pago"].indexOf(paso) >= i
                  ? "bg-emerald-500 text-white"
                  : "bg-gray-200 text-gray-500"
              }`}
            >
              {["informacion", "pago"].indexOf(paso) >= i && paso !== p ? <CheckCircle className="h-4 w-4" /> : i + 1}
            </div>
        ...
        <div className="lg:col-span-3">
            ...
            <Card className="p-6">
              ...
              <form onSubmit={handleInfoSubmit} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Input id="nombre" label="Nombre completo" value={formData.nombre} onChange={(e) => setFormData({ ...formData, nombre: e.target.value })} error={errores.nombre} />
          ...
          {paso === "pago" && (
            <Card className="p-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="font-semibold text-lg text-gray-900">Pago con Tarjeta</h2>
              </div>
              <p className="text-sm text-gray-500 mb-4">
                Datos de tarjeta de prueba: <span className="font-mono bg-gray-100 px-2 py-0.5 rounded">4242 4242 4242 4242</span>
              </p>
              <form onSubmit={handlePagoSimulado} className="space-y-4">
                <Input
                  ...
                  onChange={(e) => setTarjeta({ ...tarjeta, cardNumber: formatearTarjeta(e.target.value) })}
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <Input id="expiryYear" label="Año (AA)" placeholder="26" maxLength={4} value={tarjeta.expiryYear} onChange={(e) => setTarjeta({ ...tarjeta, expiryYear: e.target.value.replace(/\D/g, "") })} error={errores.expiryYear} />
                ...
                {resultado && !resultado.exito && (
                  <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg text-sm flex items-center gap-2">
                    <XCircle className="h-4 w-4" /> {resultado.mensaje}
                  </div>
                )}

                <div className="flex items-center justify-between pt-2">
                  <button type="button" onClick={() => setPaso("informacion")} className="text-sm text-gray-500 hover:text-indigo-600 flex items-center gap-1">
                    <ArrowLeft className="h-3 w-3" /> Volver
                  </button>
                  <Button type="submit" size="lg" isLoading={procesando}>
          ...
          {paso === "confirmacion" && (
            <Card className="p-8 text-center">
              ...
              {resultado && (
                <div className="mt-4 bg-gray-50 rounded-lg p-4 inline-block text-left">
                  <p className="text-sm text-gray-600">
                    <span className="font-medium">Transacción:</span> {resultado.transactionId}
                  </p>
                  <p className="text-sm text-gray-600">
                    <span className="font-medium">Total pagado:</span> {formatPrecio(total)}
                  </p>
                </div>
              )}
