...
export const db = {
  findMany<T extends { id: number }>(coleccion: keyof DatabaseSchema): T[] {
    const data = leerDB();
    return data[coleccion] as T[];
  },

  findById<T extends { id: number }>(
    coleccion: keyof DatabaseSchema,
    id: number
  ): T | null {
    const data = leerDB();
    const items = data[coleccion] as T[];
  ...
  findWhere<T extends { id: number }>(
    coleccion: keyof DatabaseSchema,
    predicate: (item: T) => boolean
  ): T[] {
    const data = leerDB();
    return (data[coleccion] as T[]).filter(predicate);
  },
